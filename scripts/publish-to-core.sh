#!/usr/bin/env bash
#
# Publish one or more files to the AIRBDS publication repository via a pull
# request.
#
# The shared engine behind the per-domain release scripts — it knows nothing
# about metrics or skills, only how to land files in AIBIO-UK/airbds-core
...
#
# A release is not always one file, in two different ways. --src/--dest may be
# repeated to publish a set of files that belong together — the metric ships as
# YAML and JSON renderings of the same data, and splitting them across two
# releases would let the publication repo hold two versions at once. And the
# publication repo's prose quotes the versions it ships, so that prose goes stale
# unless the release updates it too: --post-copy runs an arbitrary command inside
# the clone for exactly that, and whatever it changes is committed alongside the
# copied files. Both are why this stays generic — the engine still knows nothing
# about what is being released, only that a release is a set of changes that must
# land in one commit.
#
...
usage() {
  cat <<EOF
Usage: $(basename "$0") --src <file> --dest <path> [--src ... --dest ...] \\
                       --branch <name> [options]

Publish one or more files to root-relative <path>s of the publication
repository, on a release branch, and open a pull request.
...
Required:
  ...
  --dest <path>      Destination path within the publication repo, e.g.
                     ...
                     Both may be repeated to publish several files in one
                     commit. They are paired in the order given, so the Nth
                     --dest is where the Nth --src lands; the two must be given
...
Options:
  ...
  --post-copy <cmd>    Shell command run inside the clone after the files are
                       copied and before the commit, with the clone as its working
...
while [ $# -gt 0 ]; do
  case "$1" in
    -h|--help) usage; exit 0 ;;
    --src)            [ -n "${2:-}" ] || die "--src needs a value";  SRCS+=("$2");  shift 2 ;;
    --dest)           [ -n "${2:-}" ] || die "--dest needs a value"; DESTS+=("$2"); shift 2 ;;
...
[ "${#SRCS[@]}" -gt 0 ]  || { usage >&2; die "--src is required"; }
[ "${#DESTS[@]}" -gt 0 ] || { usage >&2; die "--dest is required"; }
[ -n "$BRANCH" ]         || { usage >&2; die "--branch is required"; }

# Paired by position, so an unequal count means the caller's intent is ambiguous
# — refuse rather than guess which file was meant to go where.
[ "${#SRCS[@]}" -eq "${#DESTS[@]}" ] ||
  die "--src and --dest must be given the same number of times, got ${#SRCS[@]} and ${#DESTS[@]}"

for i in "${!SRCS[@]}"; do
  [ -f "${SRCS[$i]}" ] || die "source file not found: ${SRCS[$i]}"
  case "${DESTS[$i]}" in
    /*|*..*) die "--dest must be a relative path inside the repo, got: ${DESTS[$i]}" ;;
  esac
  # Two sources landing on one destination would publish whichever was copied
  # last, and the loser would vanish without a word.
  for j in "${!DESTS[@]}"; do
    [ "$j" -le "$i" ] || [ "${DESTS[$j]}" != "${DESTS[$i]}" ] ||
      die "--dest given twice: ${DESTS[$i]}"
  done
done

# Joined for the default PR title, commit message, and progress output.
DEST_LIST="$(printf '%s, ' "${DESTS[@]}")"; DEST_LIST="${DEST_LIST%, }"

[ -n "$TITLE" ]      || TITLE="Publish ${DEST_LIST}"
[ -n "$COMMIT_MSG" ] || COMMIT_MSG="release: publish ${DEST_LIST}"
...
for i in "${!SRCS[@]}"; do
  mkdir -p "$(dirname "$CLONE/${DESTS[$i]}")"
  cp "${SRCS[$i]}" "$CLONE/${DESTS[$i]}"
  ...
  git -C "$CLONE" add "${DESTS[$i]}"
done

if [ -n "$POST_COPY" ]; then
  echo "==> running post-copy hook"
  # Runs after the copy so the hook sees the released files in place, and before
...
if git -C "$CLONE" diff --cached --quiet; then
  echo "==> ${CORE_REPO}:${DEST_LIST} is already identical${POST_COPY:+, and the post-copy hook changed nothing} — nothing to release"
  exit 0
fi

git -C "$CLONE" commit --quiet -m "$COMMIT_MSG"
echo "==> committed ${DEST_LIST} on ${BRANCH}"
