...
- `scripts/airbds_scoring.py` — the scoring itself: answers in, weighted score
  ...
  python3 reviews/src/scripts/airbds_scoring.py answers.json \
      --metric metric/airbds_metric_v1.0.0.json
  ```
...
- `scripts/build_review_template.py` — generates the blank template pair
  ...
  ```bash
  python3 reviews/src/scripts/build_review_template.py --version 1.0.0          # regenerate
  python3 reviews/src/scripts/build_review_template.py --version 1.0.0 --check  # verify in sync
  ```
