...
// Reference the single source of truth, the same v1.0 metric the CLI uses.
// No converter code is v1.0-specific; this suite guards that the version flows
// through end-to-end (25 questions, Ethics ABC-23…25) with no code change.
const metric = parseMetric(
  await readFile(here("../../../metric/airbds_metric_v1.0.yaml"), "utf8"),
);
const reviewCsv = await readFile(here("./fixtures/review-info-v1.0.csv"), "utf8");
const questionsCsv = await readFile(here("./fixtures/questions-v1.0.csv"), "utf8");

test("v1.0: maps reviewer and dataset fields from the review-info tab", () => {
  ...
  assert.equal(review.dataset.name, "Example v1.0 Bioscience Dataset");
...
test("v1.0: maps all 25 answers with their comments", () => {
  const { review } = convert(reviewCsv, questionsCsv, metric);
...
test("v1.0: the three ethics questions carry not_applicable; others do not", () => {
  const { review } = convert(reviewCsv, questionsCsv, metric);
  ...
  // v0.4's fourth ethics id must not leak into a v1.0 review.
  assert.equal("ABC-27" in review.answers, false);
});

test("v1.0: a fully-answered sheet produces no 'unanswered' warning", () => {
  const { warnings } = convert(reviewCsv, questionsCsv, metric);
  assert.ok(!warnings.some((w) => /Unanswered/.test(w)));
});

test("v1.0: result is left null for the processor / CI to compute", () => {
  const { review } = convert(reviewCsv, questionsCsv, metric);
  assert.equal(review.result.weighted_score, null);
  assert.equal(review.result.grade, "");
});

test("v1.0: emitted YAML quotes Yes/No and keeps schema_version a string", () => {
...
test("v1.0: detectSchemaVersion reads 1.0 from the Instructions tab", () => {
  assert.equal(detectSchemaVersion(reviewCsv), "1.0");
});
