...
suitable a bioscience dataset is for AI/ML use. The current version (v1.0.2) is a
...
`metric/airbds_metric_v1.0.2.yaml` is the **source of truth**: the canonical
...
script, which also writes a subsidiary `airbds_metric_v1.0.2.json` rendering of
the same document.

### Use the Metric

Two ways to use v1.0.2 directly:

- **[Open the Google Sheet](https://docs.google.com/spreadsheets/d/1_ZO89HEnsqaxs7HTjnDD7hZR_55K0DlisWZ5vhuyj1M/edit)** — the live editing interface, with built-in formulas that calculate the weighted score and grade automatically as you fill in answers. **Recommended for scoring a dataset today.**
- **[`airbds_metric_v1.0.2.yaml`](airbds_metric_v1.0.2.yaml)** — the source of truth, for anything programmatic (or [`airbds_metric_v1.0.2.json`](airbds_metric_v1.0.2.json), subsidiary to it, where a YAML parser is unavailable).
...
A dataset's weighted score is the sum of points for its `Yes` answers. In v1.0.2
...
versions — the summaries below are v1.0.2. (Through v1.0.0 a grade *also* had to
...
|----------|--------|---------|
| `airbds_metric_v1.0.2.yaml` | YAML | **Canonical — current.** 25-question metric: question text, grades, guidance, scopes, `instructions`, and the `grade_points`/`grading` scoring rules |
| `airbds_metric_v1.0.2.json` | JSON | **Subsidiary to the YAML.** The same v1.0.2 document, written by the same generator run, for consumers that cannot parse YAML — cite and change the YAML, not this |
| `airbds_metric_v1.0.2.upstream.json` | JSON | v1.0.2 provenance: source sheet id/url + `content_sha256` "revision" + generation timestamp |
| `airbds_metric_v1.0.1.{yaml,json,upstream.json}` | YAML/JSON | **Previous version — retained.** 25-question metric; reviews carrying `schema_version: "1.0.1"` still score against it. Same questions/weights/thresholds as v1.0.2; differs only in the reviewer `instructions` text |
| `airbds_metric_v1.0.0.{yaml,json,upstream.json}` | YAML/JSON | **Previous version — retained.** 25-question metric; reviews carrying `schema_version: "1.0.0"` still score against it. Same questions/weights/thresholds as v1.0.2, but its grading block still carries the `min_proportion_yes` gate v1.0.1 removed |
...
### How the v1.0.2 metric files are generated

From v0.4 the metric is authored in the working group's **public Google Sheet** rather than a committed `.xlsx`. `metric/src/scripts/build_metric_from_google_sheet_v1.0.2.py` pulls the Scoring, Lookups, and Header tabs and regenerates `airbds_metric_v1.0.2.yaml`, recording which sheet and a content-hash "revision" in `airbds_metric_v1.0.2.upstream.json` plus a `# Source:` breadcrumb in the YAML. (The reviewer instructions are captured verbatim into a top-level `instructions:` block — from a dedicated Instructions tab in v0.5–v1.0.0, and from the reorganised sheet's Header tab from v1.0.1 onward.) See [`metric/src/README.md`](src/README.md) for the commands, the `--check` drift check, and offline use. A weekly workflow (`.github/workflows/metric-upstream-drift-check.yml`) confirms each committed YAML still matches its Sheet, opening an issue if it has drifted. Each committed version keeps its own generator (`…_v0.4.py`, `…_v1.0.0.py`, `…_v1.0.1.py`, `…_v1.0.2.py`); the v0.3 `.xlsx` chain stays in place unchanged.
...
```bash
./metric/src/scripts/release_metric_to_core.sh 1.0.2 --dry-run   # rehearse first
./metric/src/scripts/release_metric_to_core.sh 1.0.2
```

It copies `metric/airbds_metric_v1.0.2.yaml` **and**
`metric/airbds_metric_v1.0.2.json` to the root of `airbds-core` as the
**unversioned `airbds_metric.yaml`** and **`airbds_metric.json`**, in one commit
on a `release/metric-v1.0.2` branch, and opens a pull request. It does not merge
