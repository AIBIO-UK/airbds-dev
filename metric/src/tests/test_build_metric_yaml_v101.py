#!/usr/bin/env python3
"""Offline tests for the v1.0.1 metric build script.

These run against committed fixture CSVs (the three source tabs exported from the
v1.0.1 Google Sheet), so they need no network. They cover how the sheet is read —
...
v1.0.1 reorganises the sheet from v1.0.0: the instructions moved onto a Header tab
and the Lookups grading table was retitled, so these tests also guard that the
adapted classifiers and instructions reader still pull the right content.

Run directly:
    python3 metric/src/tests/test_build_metric_yaml_v101.py
or under pytest:
    pytest metric/src/tests/test_build_metric_yaml_v101.py
"""
...
SCRIPT = REPO_ROOT / "metric" / "src" / "scripts" / "build_metric_from_google_sheet_v1.0.1.py"
COMMITTED_YAML = REPO_ROOT / "metric" / "airbds_metric_v1.0.1.yaml"
COMMITTED_JSON = REPO_ROOT / "metric" / "airbds_metric_v1.0.1.json"


def _load_build_module():
    """Import the build script by path (its filename contains a dot: v1.0.1)."""
    spec = importlib.util.spec_from_file_location("build_v101", SCRIPT)
...
def _worksheets():
    scoring = (FIXTURES / "scoring-v1.0.1.csv").read_text(encoding="utf-8")
    lookups = (FIXTURES / "lookups-v1.0.1.csv").read_text(encoding="utf-8")
    # In v1.0.1 the instructions live on the Header tab; it plays the
    # "instructions" source role the dedicated Instructions tab did in v1.0.0.
    instructions = (FIXTURES / "header-v1.0.1.csv").read_text(encoding="utf-8")
    return (bm.CsvWorksheet(scoring), bm.CsvWorksheet(lookups),
            bm.CsvWorksheet(instructions), scoring, lookups, instructions)


def test_classifiers_match_the_v101_tabs():
    """The adapted classifiers must pick out the right (renamed/moved) tabs."""
    ...
    # The Lookups classifier no longer relies on the old "Required proportions"
    # title (the v1.0.1 sheet renamed it "Threshold calculation proportions").
    assert "Required proportions" not in lookups
    # The Header tab is not mistaken for the Instructions tab of v1.0.0.
...
def test_grading_is_score_only():
    """v1.0.1 grades on total score alone; the per-category proportions (the
...
def test_read_instructions():
    *_, instructions_ws, _, _, _ = _worksheets()
    ...
    assert "\n\n" in text
    # …and excludes the 'Review information' data-entry section below it.
    assert "Reviewer name" not in text
    assert "Accession number" not in text
    # The old dedicated-tab title line is gone in v1.0.1.
    assert "AIRBDS Dataset Metric v1.0.0" not in text
...
def test_committed_yaml_regenerates_byte_for_byte():
    ...
    assert doc["schema_version"] == "1.0.1"
    ...
    assert ss.strip_source_breadcrumb(generated) == ss.strip_source_breadcrumb(committed), (
        "committed metric/airbds_metric_v1.0.1.yaml is out of sync with the fixtures; "
