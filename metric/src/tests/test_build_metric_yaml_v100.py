#!/usr/bin/env python3
"""Offline tests for the v1.0.0 metric build script.
...
Run directly:
    python3 metric/src/tests/test_build_metric_yaml_v100.py
or under pytest:
    pytest metric/src/tests/test_build_metric_yaml_v100.py
"""
...
SCRIPT = REPO_ROOT / "metric" / "src" / "scripts" / "build_metric_from_google_sheet_v1.0.0.py"
COMMITTED_YAML = REPO_ROOT / "metric" / "airbds_metric_v1.0.0.yaml"
COMMITTED_JSON = REPO_ROOT / "metric" / "airbds_metric_v1.0.0.json"


def _load_build_module():
    """Import the build script by path (its filename contains a dot: v1.0.0)."""
    spec = importlib.util.spec_from_file_location("build_v100", SCRIPT)
...
def _worksheets():
    scoring = (FIXTURES / "scoring-v1.0.0.csv").read_text(encoding="utf-8")
    lookups = (FIXTURES / "config-v1.0.0.csv").read_text(encoding="utf-8")
    instructions = (FIXTURES / "instructions-v1.0.0.csv").read_text(encoding="utf-8")
...
def test_committed_yaml_regenerates_byte_for_byte():
    ...
    assert ss.strip_source_breadcrumb(generated) == ss.strip_source_breadcrumb(committed), (
        "committed metric/airbds_metric_v1.0.0.yaml is out of sync with the fixtures; "
