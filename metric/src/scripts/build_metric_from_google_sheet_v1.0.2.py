#!/usr/bin/env python3
"""Build the canonical v1.0.2 metric files from the AIRBDS Google Sheet.

Like v0.4, v0.5, v1.0.0, and v1.0.1 before it, v1.0.2 is authored in a public
Google Sheet (the working group's live doc). This script pulls the Scoring,
Lookups, and Instructions tabs via the public CSV export (the sheet's own export
format, not this script's output format) and regenerates the metric files from
them.

Two renderings of the same metric are written:

  * airbds_metric_v1.0.2.yaml — the canonical, human-readable form. Carries the
    explanatory comments and is what people read and review.
  * airbds_metric_v1.0.2.json — the same content for consumers that must run
...
Usage:
    # Regenerate metric/airbds_metric_v1.0.2.yaml from the live sheet
    python3 metric/src/scripts/build_metric_from_google_sheet_v1.0.2.py

    # Verify the committed file still matches the live sheet (the drift check)
    python3 metric/src/scripts/build_metric_from_google_sheet_v1.0.2.py --check
...
Schema notes (v1.0.2):
  * v1.0.2 is a guidance-only PATCH over v1.0.1. The 25 questions, their ids,
    scopes, grades, and per-question guidance, the weights, and the numeric score
    thresholds are all unchanged; only the top-level reviewer `instructions:`
    text changed, gaining a sentence directing reviewers to download at least a
    sample of the dataset for testing during the assessment. It is authored in
    its own sheet (a copy of the v1.0.1 sheet with the guidance clarification
    applied), so v1.0.1 stays reproducible from its own source.
  * The source sheet keeps the v1.0.1 shape: grading is by total score alone (no
    `min_proportion_yes`); the per-category proportions in the sheet ("Threshold
    calculation proportions") remain the author's working for deriving the score
    thresholds, not a scoring rule, and are read past.
  ...
  * Per-question points live in the Lookups tab's 'COUNTA of Grade' pivot, in
    its 'Points per Question' column; the grading table below it carries the
    grading thresholds. The grading/points readers key off the pivot's column
    headers, so they are unchanged from v1.0.1.
  * The generic reviewer instructions live on the Header tab, in an
    'Instructions:' cell above a 'Review information' data-entry block (as in
    v1.0.1). That cell is captured verbatim into a top-level
...
DEFAULT_SHEET = "https://docs.google.com/spreadsheets/d/1_ZO89HEnsqaxs7HTjnDD7hZR_55K0DlisWZ5vhuyj1M/edit"
DEFAULT_OUTPUT = REPO_ROOT / "metric" / "airbds_metric_v1.0.2.yaml"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build the v1.0.2 metric YAML + JSON from the Google Sheet.")
