...
def main() -> None:
    ...
    if args.check:
        ...
        # source-hash breadcrumb is set aside for the comparison because it tracks
        # the raw CSV and shifts for edits the generator never reads — an unread
        # cell, trailing whitespace. Comparing it would report drift for a metric
        # that is unchanged. Same rule as the v0.5 generator.
        committed = args.output.read_text(encoding="utf-8") if args.output.exists() else ""
        if strip_source_breadcrumb(committed) != strip_source_breadcrumb(generated_yaml):
        ...
        # metric does not reach. The committed file is left alone rather than
        # churning a commit whose only change is a hash line.
        recorded = recorded_source_sha(committed)
