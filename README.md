...
[![Version](https://img.shields.io/badge/metric%20version-v1.0.2-blue)](CHANGELOG.md)
...
Two ways to use the current metric (v1.0.2) directly — pick whichever suits you:

| | Link | Best for |
|---|---|---|
| 📄 **YAML file** (source of truth) | [`metric/airbds_metric_v1.0.2.yaml`](metric/airbds_metric_v1.0.2.yaml) | Scripting, tooling, or anything that reads the metric programmatically |
| 📊 **Google Sheet** (editing interface) | [Open the sheet](https://docs.google.com/spreadsheets/d/1_ZO89HEnsqaxs7HTjnDD7hZR_55K0DlisWZ5vhuyj1M/edit) | Browsing, filtering, or copying into your own spreadsheet — no coding required |
...
A JSON rendering, [`metric/airbds_metric_v1.0.1.json`](metric/airbds_metric_v1.0.1.json),
...
└── scripts/   # Helper scripts that span the whole repo (diagram rendering, publishing to airbds-core)
```
...
[`metric/airbds_metric_v1.0.1.yaml`](metric/airbds_metric_v1.0.1.yaml).
...
- **MINOR** — question additions, deletions, or rewordings (e.g. v1.1.0)
- **MAJOR** — changes to weights or grade thresholds (e.g. v2.0.0)

All three components are written out, including trailing zeros: the metric is
`v1.0.0`, not `v1.0`. The string is a path component and a manifest key matched
exactly, so pick one shape and keep it. The retained v0.4 and v0.3 metrics keep
their two-part names — four committed reviews carry those exact strings in
`schema_version`, so renaming them would orphan the reviews.
...
> **v1.0.2 is the current version.** The metric, the review templates, and the
> sheet→YAML converter target v1.0.2; the assessment skill channels
> (`development`, `testing`, `production`) are repointed to it as part of this
> release. v1.0.1 and v1.0.0 are **retained** — superseded but kept so reviews
> scored against them can be re-scored — as are v0.4 and v0.3; the version-aware
> review processor scores each review against the metric matching its
> `schema_version`. v0.5 was **withdrawn** when v1.0.0 superseded it: the two are
> the same metric, and no review was ever scored against v0.5. See the `[1.0.2]`
> entry under **Metric** in [CHANGELOG.md](CHANGELOG.md#metric).
